...
import { Apple, MapPin, SlidersHorizontal } from "lucide-react";
...
export default function Fruits() {
  ...
  const filtered = activeFilter === "All"
    ? allFruits
    : allFruits.filter((f) => f.freshness === activeFilter);
          ...
          {loading ? (
            ...
            >
              {filtered.map((product) => (
                <div
                  key={product.id}
                  className="group rounded-2xl overflow-hidden transition-all duration-300 border-2 shadow-lg bg-card border-border shadow-sm hover:shadow-xl"
                >
